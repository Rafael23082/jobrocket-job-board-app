...
function TagsInputBox({formValues, setFormValues, field, index}){
    ...
    useEffect(() => {
        let fieldOptions;
        if (currentTag.trim() != ""){
            fieldOptions = field.options.filter((option) => option.toLowerCase().includes(currentTag.toLowerCase()));
        }else{
            fieldOptions = []
        }
    ...
    return(
        <div className="relative">
            ...
            <div className="mt-[0.4em] border w-[100%] rounded-[5px] focus-within:border focus-within:outline">
                <div className="flex flex-wrap py-[1em] items-center">
                    <div className="px-[1em] flex flex-wrap gap-x-[0.7em] gap-y-[0.5em]">
            ...
            {suggestionsDisplayed && (
                <div className={`flex flex-col absolute top-[calc(100%+10px)] w-[100%] rounded-[10px] select-none overflow-hidden border shadow bg-white`}>
