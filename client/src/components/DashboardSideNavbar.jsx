    ...
    return(        
        ...
        <div className={`${placeholder && "invisible hidden lg:block relative"} hidden lg:flex flex-col fixed w-fit pb-[1em] h-[100vh] top-0 left-0 bg-white justify-between overflow-y-auto shadow z-10`} style={{fontFamily: "'Roboto', sans-serif"}}>
            <div className="flex flex-col text-[0.9rem]">
                ...
                <div className={`mt-[3em] py-[1em] mx-[0.5em] ${current == "Dashboard" ? "bg-gray-100": "bg-white text-gray-500"} rounded-[5px] pl-[1em] cursor-pointer flex items-center gap-x-[0.8em]`} onClick={() => {navigate(`/dashboard`)}}><FiMonitor size={16} color="#3B82F6" /> Dashboard</div>
            ...
            <div className={`py-[1em] text-[0.9rem] mx-[0.5em] pl-[1em] ${current == "Settings" ? "bg-gray-100": "bg-white text-gray-500"} rounded-[5px] cursor-pointer flex items-center gap-x-[0.8em]`} onClick={() => navigate(`/dashboard/profile`)}><IoSettings size={16} color="#000000" /> Settings</div>
        </div>
